package com.znw.mydemo.utils.device;
...
public class NetUtil {
    ...
    public static void getScanWifiResults(Context context) {
        ...
        for (ScanResult wifi : wifiResults) {
            LogUtils.debug(TAG,  wifi.toString());
        }

        TelephonyManager tm = (TelephonyManager) context.getSystemService(Context.TELEPHONY_SERVICE);


        LogUtils.debug(TAG,  TelephonyManager.PHONE_TYPE_GSM + "----" + tm.getPhoneType());
        List<NeighboringCellInfo> cellResults = tm.getNeighboringCellInfo();
        for (NeighboringCellInfo cell : cellResults) {
        	 LogUtils.debug(TAG,  cell.getCid() + "-" + cell.getLac() + "-" + cell.getRssi() + "-" + cell.getPsc() + "-" + cell.getNetworkType());
        }

        LogUtils.debug(TAG,  getProviderName(context).getText());
    }
